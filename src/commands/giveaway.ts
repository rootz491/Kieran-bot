...
export default {
  ...
  async execute(interaction: CommandInteraction) {
    try {
      ...
      // @ts-ignore
      const prize = interaction.options.getString('prize') ?? 'No prize provided';
      // @ts-ignore
      const winners = interaction.options.getInteger('winners') ?? 1;
      // @ts-ignore
      const length = interaction.options.getString('ends') ?? 'No length provided';

      if (length.split('/').length !== 3) {
        await interaction.reply({
          content: 'Invalid length format. Please use d/h/s (Days, Hours, Seconds).',
          ephemeral: true,
        });
        return;
      } else {
      ...
      const embed = new EmbedBuilder()
        .setTitle(name)
        .setDescription(`Prize: ${prize}\nWinners: ${winners}\nHosted by: ${interaction.user}\nEnds in <t:${Math.floor(endMs / 1000)}:f>\n\nReact with 🎉 to enter!`)
        .setTimestamp();
